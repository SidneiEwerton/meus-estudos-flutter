...
static String? nomeCadastro(String? value){
  final nomeRegistro = value?.trim();
  if(nomeRegistro == null || nomeRegistro.isEmpty){
    return 'insira o nome ';
  } 
  final name = RegExp(r'^[a-zA-Z]{4,20}$');
  if(!name.hasMatch(nomeRegistro)){
    return 'Deve conter de 4 a 10 letras';
  }
  return null;
}
...
// validação do cargo
static String? cargoAtual(String? value){
  final cargoValue = value?.trim();
  if(cargoValue == null || cargoValue.isEmpty){
    return 'Preencher o cargo';
  }
  final regExp = RegExp(r'^[a-zA-Z]{4,10}');
  if(!regExp.hasMatch(cargoValue)){
    return 'Deve conter de 4 a 10 letras';
  }
  return null;
}
...
static String? setorAtual(String? value){
  final setorAtual = value?.trim();
  if(setorAtual == null || setorAtual.isEmpty){
    return 'Entre com o nome do setor';
  }
  final local = RegExp(r'^[A-zA-Z]{4,20}$');
  if(!local.hasMatch(setorAtual)){
  return 'deve conter de 4 a 10 letras';
  }
  return null;
}
